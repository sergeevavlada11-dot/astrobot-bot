...
async def final_generate(message: types.Message):
    ...
    if sphere == "🧬 Личность":
        prompt = (
            ...
            f"Подтема: {sub_text}\n\n"
            "⚠️ Важно: Используй именно приведённые выше данные карты. "
            "Не описывай в общем, что означает Асцендент или 10-й дом — указывай конкретно, в каком знаке и доме находятся планеты у этого человека, "
            "какие управители и аспекты задействованы, и как это проявляется в его жизни. "
            "Приводи конкретные выводы и прогнозы, основываясь на этих положениях. "
            "Если в карте Солнце в 5 доме во Льве — анализируй именно это положение. "
            "Если Асцендент в Раке — покажи, как это влияет на стиль работы именно ЭТОГО человека. "
