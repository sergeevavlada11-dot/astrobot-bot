...
async def final_generate(message: types.Message):
    ...
    birth_text = (
        ...
        f"⏰ Время рождения: {birth.get('birth_time','—')}"
    )

    sphere_text = SPHERE_MAP[sphere]
    sub_text = SUB_MAP[sub]

    topics = main_topics.get(sphere, [])

 prompt = (
    "Ты — профессиональный ведический астролог (джйотиш) с глубоким знанием кармы, дхармы и судьбы.\n"
    "Пиши на русском языке в спокойном, уважительном и осознанном тоне, как будто даёшь личную консультацию клиенту.\n"
    "Используй ключевые понятия джйотиш (Лагна, дома, грахи, накшатры, кармические узлы), но обязательно объясняй их простыми словами.\n"
    "Стиль ответа: структурированный, философский, с минимумом эмодзи (только в заголовках или советах).\n"
    "Фокус: раскрыть глубинный смысл жизни человека, его задачи и потенциал через призму кармы.\n"
    "В конце каждого раздела добавь 2–3 практических рекомендации из ведической традиции (например, мантры, методы гармонизации планет, дни силы, советы по дхарме).\n\n"
    "📜 Исходные данные:\n"
    f"{birth_text}\n\n"
    f"📚 Сфера анализа: {sphere_text}\n"
    f"🔎 Подраздел: {sub_text}\n\n"
...
prompt += (
    "\n📌 Структура ответа:\n"
    "• Краткое вступление (1–2 абзаца о значении этой сферы в контексте дхармы)\n"
    "• Подробный анализ по каждому пункту (каждый пункт — отдельный раздел с заголовком длиною 7-10 предложений)\n"
    "• Итог: вывод и 2–3 практических шага для гармонизации и раскрытия потенциала\n\n"
    "❗ Избегай банальных фраз и не пиши общие гороскопы. Говори как эксперт, консультирующий лично этого человека."
)

    try:
        completion = client.chat.completions.create(
            model="gpt-4",  # можно заменить на gpt-4o если доступен
            messages=[
                {"role": "system", "content": "Ты опытный астролог-консультант. Пиши дружелюбно, с эмодзи и глубокой структурой."},
        ...
        await message.answer("⚠️ Сейчас ИИ недоступен. Попробуй чуть позже.")
    except Exception:
