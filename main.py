...
async def final_generate(message: types.Message):
    ...
    prompt = (
        ...
        "Твоя задача — подготовить развёрнутый и глубокий персональный астрологический анализ на русском языке.\n"
        "Стиль: дружелюбный, структурированный, с эмодзи. Без страшилок и фатализма.\n"
        "Фокус: раскрыть потенциал человека и дать ему практические советы для роста и реализации.\n\n"
        "📜 Данные рождения:\n"
        f"{birth_text}\n\n"
    ...
    prompt += (
        "\n📌 Структура ответа:\n"
        "• Краткое введение (2–3 предложения о значении этой сферы в жизни человека)\n"
        "• Анализ по каждому пункту (каждый пункт — отдельный подзаголовок с эмодзи)\n"
        "• Итоговый вывод и 2–3 практических совета 🚀\n\n"
        "❗ Не используй сухую теорию. Пиши конкретно, как будто даёшь консультацию лично этому человеку.\n"
    )
